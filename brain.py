...
class AppSession(ApplicationSession):
	...
	status = {
        ...
        def modulate_temperature():
            ...
            print type(status['control'])
            if status['control']:
                if status["cur_temp"] <= status["min_temp"]:
                    if status['power'] != 'ON':
        ...
        def record():
            self.log.debug("recording data", status = status)
            # record the progressive statuses
            # fill up an array
            # if the array is > 100
            # # write to file
            pass
	...
	@inlineCallbacks
	    ...
	    status['cur_temp'] = int(msg['t'])
	    status['cur_hum'] = int(msg['h'])
